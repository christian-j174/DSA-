...
public class RoadTripWrapper {
	...
	public static class City {
		...
		public String toString() {
			return "(" + this.getCityName() + ", " + this.getTollCost() + ")";
		}
...
public static int canTravelMinCost(String A, String B, Graph<String, City> G) {
    // Explicitly specify type parameters for HeapPriorityQueue
    PriorityQueue<Integer, City> queue = new HeapPriorityQueue<>(new Comparator<Integer>() {
        @Override
    ...
    queue.insert(0, new City(A, 0));

    while (!queue.isEmpty()) {
        Entry<Integer, City> entry = queue.removeMin();
        City currentCity = entry.getValue();
        int currentCost = entry.getKey();

        if (currentCity.getCityName().equals(B)) {
            return currentCost; 
        }

        if (!visited.contains(currentCity.getCityName())) {
            visited.add(currentCity.getCityName());

            List<City> neighbors = G.getNeighbors(currentCity.getCityName());
            for (City neighbor : neighbors) {
                if (!visited.contains(neighbor.getCityName())) {
                    int newCost = currentCost + neighbor.getTollCost();
                    queue.insert(newCost, new City(neighbor.getCityName(), newCost));
                }
            }
        }
    }
