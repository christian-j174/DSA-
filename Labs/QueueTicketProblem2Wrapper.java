...
public class QueueTicketProblem2Wrapper {
	...
	public static int satisfiedCustomers(int[] ticketNeeds, int availableTickets) {
        DoublyLinkedQueue<Integer> ticketQueue = new DoublyLinkedQueue<>(); 
        int clientSatisfied = 0;
        int tmp = 0;
        
        //Add people to Queue
        for(int i=0; i < ticketNeeds.length; i++)
            ticketQueue.enqueue(ticketNeeds[i]);
        
        while(!ticketQueue.isEmpty() || availableTickets < 0){
            tmp = ticketQueue.dequeue(); 
            ...
            // Ya tiene todas sus taquillas 
            if(tmp == 0){
                clientSatisfied++;
            }
            // Se le dio una taquilla y vuelve de nuevo a la fila
            else {
				ticketQueue.enqueue(tmp);
            }
        }
