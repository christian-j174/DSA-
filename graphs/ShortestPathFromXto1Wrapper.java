...
public class ShortestPathFromXto1Wrapper {
...
public static int convertXToOne(int X) {
    Queue<OrderedPair<Integer, Integer>> queue = new DoublyLinkedQueue<>();
    List<Integer> visited = new LinkedList<>();

    queue.enqueue(new OrderedPair<>(X, 0));
    visited.add(X);

    while (!queue.isEmpty()) {
        OrderedPair<Integer, Integer> pair = queue.dequeue();
        int currentNumber = pair.getFirst();
        int operations = pair.getSecond();

        if (currentNumber == 1) {
            return operations;
        }

        if (!isVisited(visited, currentNumber - 1)) {
            queue.enqueue(new OrderedPair<>(currentNumber - 1, operations + 1));
            visited.add(currentNumber - 1);
        }

        if (currentNumber % 2 == 0 && !isVisited(visited, currentNumber / 2)) {
            queue.enqueue(new OrderedPair<>(currentNumber / 2, operations + 1));
            visited.add(currentNumber / 2);
        }

        if (currentNumber % 3 == 0 && !isVisited(visited, currentNumber / 3)) {
            queue.enqueue(new OrderedPair<>(currentNumber / 3, operations + 1));
...
private static boolean isVisited(List<Integer> visited, int number) {
    return visited.contains(number);
}
