...
public class SLToBSTWrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		public boolean isBalanced() {
			if (this.isEmpty() ||this.size() == 1) {
				...
				return Math.abs(this.heightAux(this.root.getLeftChild()) - this.heightAux(this.root.getRightChild())) <= 1; 
			}
		}
	...
	/*
	 * Write a non-member method named sortedListToBST. Given a singly linked list where elements are sorted 
	 * in ascending order, convert it to a height balanced Binary Search Tree BST.
	 * The method receives as parameter a non-empty list with integers sorted in increasing (ascending) order.  
	 * Note: The tester will use BST method isBalanced() (see above) to check if the tree is balanced.
	 * 
	 * Hint 1: Recall binary search on an array of integers. 
	 * 
	 * Hint 2: Write an auxiliary function that moves over the singly-linked list as if doing a binary search: 
	 * split the list in halves and always put in the BST the element in the midpoint of the list segment being processed. 
	 * Divide and Conquer.  
	 *
	 */
	public static BinarySearchTree<Integer, Integer> sortedListToBST(SinglyLinkedList<Integer> list) {
		return new BinarySearchTree<>(new IntegerComparator(), sortedListToBSTAux(list, 0, list.size() - 1));
	}
	
	private static BinaryTreeNodeImp<MapEntry<Integer, Integer>> sortedListToBSTAux(SinglyLinkedList<Integer> list, int start, int end) {
		if (start > end) {
			return null;
		}
		...
		// Create a new node with the midValue
		BST.SLToBSTWrapper.BinarySearchTree.MapEntry<Integer, Integer> mapEntry = new BST.SLToBSTWrapper.BinarySearchTree.MapEntry<>(midValue, midValue);
		BinaryTreeNodeImp<BST.SLToBSTWrapper.BinarySearchTree.MapEntry<Integer, Integer>> node = new BinaryTreeNodeImp<>(mapEntry, null, null, null);
	
		// Construct the left and right subtrees recursively
		node.setLeftChild(sortedListToBSTAux(list, start, mid - 1));
		node.setRightChild(sortedListToBSTAux(list, mid + 1, end));
