...
public class Lab06P1Wrapper {
	...
	public static class DoublyLinkedList<E> implements List<E> {
		...
		public void add(E obj) {
			Node nextNode = this.trailer;  //atras
			Node prevNode = this.trailer.getPrev();  //alfrente
		...
		public int removeAll(E obj) {
		    ...
		    while (nextNode != trailer) {
		        if (nextNode.getValue().equals(obj)) {
		            ...
		            Node afterNextNode = nextNode.getNext();

		            if (prevNode != null) {
		                prevNode.setNext(afterNextNode);
		            }
		            if (afterNextNode != null) {
		                afterNextNode.setPrev(prevNode);
		            }

		            // Clear the node and increase the counter
		            nextNode.clear();
		            currentSize--;
		            counter++;
