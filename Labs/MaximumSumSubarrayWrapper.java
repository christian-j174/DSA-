...
public class MaximumSumSubarrayWrapper {
public interface Queue<E> {
	...
	public static int maxSumSubarray(int[] array, int window_size) {
		int sum = 0;
		DoublyLinkedQueue<Integer> window = new DoublyLinkedQueue<>();
		int windowTotal = 0;
		...
		for(int i = window_size; i < array.length; i++){
			...
			windowTotal -= window.dequeue();

			sum = Math.max(sum, windowTotal);
		}

		if(array == null || array.length < window_size){
	...
	public static void main(String[] args){
		int arg[] = {2, 1, 5, 1, 3, 2};
		System.out.println(maxSumSubarray(arg, 3));
	}
}